...
describe('release', () => {
  ...
  it('smoke test / map', async () => {
    const stream = from(sequence(3)).pipe(map((value) => value + 100));

    const outputs = await toArray(stream);

    expect(outputs).toEqual([100, 101, 102]);
  });
  it('smoke test / scale', async () => {
    const stream = from(sequence(3)).pipe(scale(1, (value) => value + 100));

    const outputs = await toArray(stream);

    expect(outputs.sort()).toEqual([100, 101, 102]);
  });
});
